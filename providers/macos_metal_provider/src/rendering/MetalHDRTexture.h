...
namespace arv {
    ...
    class MetalHDRTexture2D : public Texture2D {
    public:
#ifdef __OBJC__
        MetalHDRTexture2D(id<MTLDevice> device, const std::string& path);
#else
        MetalHDRTexture2D(void* device, const std::string& path);
#endif
        ~MetalHDRTexture2D();
